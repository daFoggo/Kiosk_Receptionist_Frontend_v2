...
const HomeWebcam = memo(({ isConnected, onFrameCapture, webcamData }: IHomeWebcamProps) => {
        ...
        >
          ...
          >
            ...
            <span className="sr-only">
              {isConnected ? "Connected" : "Disconnected"}, {totalDetectedPeople} people detected
            </span>
          </Badge>
        </motion.div>
        ...
        >
          <div className="flex space-x-2 min-w-full">
            <AnimatePresence mode="popLayout">
              {!webcamData?.main && (!webcamData?.others || webcamData.others.length === 0) && (
                <motion.div
                  key="skeleton"
              ...
              {webcamData?.main && (
                <PersonBadge key={`main-${webcamData.main.name}`} person={webcamData.main} isMain />
              )}

              {webcamData?.others?.map((person, index) => (
                <PersonBadge key={`other-${person.name}-${index}`} person={person} />
              ))}
            </AnimatePresence>
          </div>
        </motion.div>
